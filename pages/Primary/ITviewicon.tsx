    ...
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="mb-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 bg-white p-8 rounded-[3rem] border border-slate-200 shadow-sm">
         <div className="flex items-center gap-6">
            <button onClick={onBack} className="h-12 w-12 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-slate-400 hover:text-emerald-600 transition-all shadow-sm">
            ...
            <div className="min-w-0">
               <h2 className="text-3xl font-black text-slate-900 uppercase tracking-tight truncate leading-none">{client.legalName}</h2>
               <p className="text-[10px] font-black text-emerald-500 uppercase tracking-[0.4em] mt-3">Direct Tax Vault Profile</p>
            </div>
         </div>
         <div className="px-5 py-2.5 rounded-2xl bg-emerald-50 border border-emerald-100 flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="lg:col-span-2 space-y-8">
            <section className="bg-white rounded-[2.5rem] border border-slate-200 p-10 shadow-sm space-y-10">
               <h4 className="text-[11px] font-black uppercase tracking-[0.25em] text-emerald-600 flex items-center gap-3">Assessment Identity <div className="h-px flex-1 bg-slate-100" /></h4>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
                  ...
                  <div className="space-y-1"><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Category</p><p className="text-lg font-black text-slate-900 uppercase">{client.itProfile?.category}</p></div>
                  <div className="space-y-1"><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Father's / Partner Name</p><p className="text-lg font-black text-slate-900 uppercase">{client.itProfile?.fatherName || '---'}</p></div>
                  <div className="space-y-1"><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Date of Birth / Inc.</p><p className="text-lg font-black text-slate-900">{client.itProfile?.dob || '---'}</p></div>
               </div>
            </section>
            ...
            <section className="bg-slate-900 rounded-[2.5rem] p-10 text-white shadow-2xl relative overflow-hidden">
               ...
               <h4 className="text-[11px] font-black uppercase tracking-[0.25em] text-emerald-400 mb-10 flex items-center gap-3 relative z-10">E-Filing Portal Access <div className="h-px flex-1 bg-white/10" /></h4>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                  <div className="space-y-2">
                     <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Portal User ID</p>
                     <p className="text-2xl font-black uppercase tracking-tight">{client.itProfile?.username}</p>
                  </div>
                  <div className="space-y-2">
                     <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest">Secured Password</p>
                     <p className="text-2xl font-black text-emerald-400 tracking-[0.2em]">••••••••</p>
                  </div>
               </div>
            </section>
         </div>
         ...
         <div className="space-y-8">
            <section className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm space-y-6">
               ...
               <div className="space-y-5">
                  ...
                  {client.itProfile?.businessName && (
                    <div>
                       <p className="text-[9px] font-black uppercase text-slate-400 mb-1">Business Identity</p>
                       <p className="text-sm font-black text-slate-800 uppercase">{client.itProfile.businessName}</p>
                    </div>
                  )}
               </div>
            </section>
            ...
            <section className="bg-white rounded-[2.5rem] border border-slate-200 p-8 shadow-sm space-y-6">
               ...
               <div className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-50 rounded-2xl">
                     <p className="text-[9px] font-black uppercase text-slate-400 mb-1">Work Email</p>
                     <p className="text-sm font-bold text-slate-600 lowercase truncate">{client.email}</p>
                  </div>
               </div>
            </section>
         </div>
