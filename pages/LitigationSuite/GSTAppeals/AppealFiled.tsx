...
const AppealFiled: React.FC = () => {
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      console.error("Appeal Sync Failed:", err);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const updateHearingDate = async (record: LitigationRecord, date: string) => {
    ...
    } catch (err) { alert("Hearing date update failed."); }
  };
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1550px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[180px]">Next Hearing</th>
