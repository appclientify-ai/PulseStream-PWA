...
const AppealDemand: React.FC = () => {
  ...
  const handleExportCSV = () => {
    const headers = ['S.No.', 'Trade Name', 'GSTIN', 'Order Ref', 'Section', 'Demand Date', 'Status'];
    ...
    exportToCSV(headers, rows, 'GST_Appeal_Demands.csv');
  };

  const handleExportPDF = () => {
    const headers = ['Trade Name', 'GSTIN', 'Order Ref', 'Section', 'Demand Date', 'Status'];
    ...
    printList('GST Appeal Sustained Demands', headers, rows);
  };
    ...
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-3 bg-white p-2.5 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 group w-full">
          <input 
            type="text" 
            placeholder="Search sustained appeal demands by Trade Name, GSTIN or Order Ref..." 
            value={search} 
          ...
          <div className="p-4 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredRecords.length === 0 ? (
              <div className="col-span-full py-24 text-center text-slate-400 font-bold uppercase tracking-wider text-xs">
                No sustained appeal demands found
              </div>
            ) : (
                        ...
                        {activeStatusMenuId === rec.id && (
                          <div className="absolute bottom-full mb-1 z-50 left-0 w-44 bg-white border border-slate-200 rounded-xl shadow-xl p-1 text-left flex flex-col">
                            ...
                            <button onClick={() => {
                              ...
                              setIsModalOpen(true);
                              setActiveStatusMenuId(null);
                            }} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-indigo-50 text-indigo-700 text-left border-t border-slate-50 cursor-pointer">GSTAT (Tribunal)</button>
                            <button onClick={() => {
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1100px]">
              <thead className="sticky top-0 z-30 bg-slate-100">
                <tr className="bg-slate-50 border-b border-slate-200 shadow-sm font-bold uppercase tracking-wider text-slate-900 text-xs">
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[14%] min-w-[110px] whitespace-nowrap">Appeal U/s</th>
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[15%] min-w-[130px] text-center whitespace-nowrap">Next Phase Update</th>
                ...
                {filteredRecords.length === 0 ? (
                  <tr>
                    <td colSpan={7} className="py-24 text-center text-slate-400 font-bold uppercase tracking-wider">
                      No sustained appeal demands documented
                    </td>
                  </tr>
                ) : (
                  filteredRecords.map((rec, idx) => {
