...
const TribunalDrop: React.FC = () => {
  ...
  const records = useMemo(() => allRecords.filter(r => r.status === 'Drop'), [allRecords]);
  ...
  return (
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-4 overflow-hidden">
      ...
      <div className="flex-1 bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        <div className="overflow-x-auto no-scrollbar flex-1 pb-32">
                ...
                filteredRecords.map((rec, idx) => {
                  ...
                  return (
                    <tr key={rec.id} className="hover:bg-indigo-50/20 transition-all group text-[12px]">
                      ...
                      <td className=" px-6 py-5 font-black text-emerald-600 uppercase">{formatDisplayDate(rec.orderDate || rec.aioDate || rec.oioDate || rec.issuedDate)}</td>
           ...
           <div className="w-full max-w-2xl bg-white rounded-[2rem] shadow-2xl flex flex-col animate-in zoom-in-95 max-h-[90vh] overflow-hidden">
              <div className="px-10 py-8 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
                 <div className="min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">Tribunal Case Relief Profile</p>
                 </div>
              ...
              <div className="p-10 grid grid-cols-2 gap-8 flex-1 overflow-y-auto no-scrollbar">
                 ...
                 <div><p className="text-[10px] font-black uppercase text-slate-400 mb-1">Order/Judgment Date</p><p className="text-base font-black text-emerald-600">{formatDisplayDate(viewingRecord.orderDate)}</p></div>
                 <div><p className="text-[10px] font-black uppercase text-slate-400 mb-1">Order/Relief Ref No</p><p className="text-base font-black text-slate-900">{viewingRecord.referenceNo || viewingRecord.replyReferenceNo || "---"}</p></div>
                 <EditableCaseHistory 
                 ...
                 />
                 <div className="col-span-2 bg-slate-50 p-6 rounded-2xl border border-slate-100"><p className="text-[10px] font-black uppercase text-slate-400 mb-2">Staff Remarks & Case History</p><p className="text-sm font-medium text-slate-600 italic leading-relaxed">{viewingRecord.remarks || 'No notes found.'}</p></div>
              </div>
