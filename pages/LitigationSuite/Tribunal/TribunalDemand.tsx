  ...
  const filteredRecords = useMemo(() => {
    ...
    return records.filter(rec => {
      ...
      return (rec.clientName || '').toLowerCase().includes(s) || 
             ((rec.aioArn || rec.oioRefNo || rec.referenceNo || rec.filingNo || '').toLowerCase().includes(s)) ||
             (client?.gstProfile?.gstin || '').toLowerCase().includes(s);
    });
  }, [records, clients, search]);

  const handleExportCSV = () => {
    const headers = ['S.No.', 'Trade Name', 'GSTIN', 'Case Ref / AIO ARN', 'Section', 'Order Date', 'Status'];
    const rows = filteredRecords.map((r, idx) => {
      ...
      return [
        ...
        r.clientName,
        c?.gstProfile?.gstin || '---',
        r.aioArn || r.oioRefNo || r.referenceNo || r.filingNo || '---',
        'U/s ' + (r.section || '---'),
        r.orderDate || r.tioDate || r.issuedDate || '---',
        'Sustained Demand'
      ];
    });
    exportToCSV(headers, rows, 'GSTAT_Tribunal_Demands.csv');
  };

  const handleExportPDF = () => {
    const headers = ['Trade Name', 'GSTIN', 'Case Ref / AIO ARN', 'Section', 'Order Date', 'Status'];
    ...
    printList('GSTAT Tribunal Sustained Demands', headers, rows);
  };
    ...
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-3 bg-white p-2.5 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 group w-full">
          <input 
            type="text" 
            placeholder="Search sustained tribunal matters by Trade Name, GSTIN or Case Ref..." 
            value={search} 
          ...
          <div className="p-4 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredRecords.length === 0 ? (
              <div className="col-span-full py-24 text-center text-slate-400 font-bold uppercase tracking-wider text-xs">
                No sustained tribunal demands found
              </div>
            ) : (
                  ...
                  <div key={rec.id} className="p-4 bg-slate-50/70 hover:bg-white border border-slate-200 rounded-2xl shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-3">
                    <div>
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-md bg-rose-50 text-rose-700">
                          #{((idx + 1).toString().padStart(2, '0'))}
                        </span>
                        <span className="text-xs font-bold text-rose-600">
                          Order: {formatDate(rec.aioDate || rec.oioDate || rec.orderDate || rec.issuedDate)}
                        </span>
                      </div>
                    ...
                    <div className="bg-white p-2.5 rounded-xl border border-slate-200/70 space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center justify-between text-slate-600">
                        ...
                        <span className="font-medium text-slate-700">{formatDate(rec.aioDate || rec.oioDate || rec.orderDate || rec.issuedDate)}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-200/60">
                      <div className="relative">
                        <button 
                          ...
                          className="px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 transition-all flex items-center gap-1 cursor-pointer"
                        >
                          <span>Actions</span>
                        ...
                        {activeStatusMenuId === rec.id && (
                          <div className="absolute bottom-full mb-1 z-50 left-0 w-44 bg-white border border-slate-200 rounded-xl shadow-xl p-1 text-left flex flex-col">
                            <button onClick={() => updateRecordStatus(rec, 'Drop', true)} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-amber-50 text-amber-700 text-left cursor-pointer">Demand Paid</button>
                            <button onClick={() => {
                              ...
                              setIsModalOpen(true);
                              setActiveStatusMenuId(null);
                            }} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-purple-50 text-purple-700 text-left border-t border-slate-50 cursor-pointer">High Court (WP)</button>
                          </div>
                        )}
                      </div>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1100px]">
              <thead className="sticky top-0 z-30 bg-slate-100">
                <tr className="bg-slate-50 border-b border-slate-200 shadow-sm font-bold uppercase tracking-wider text-slate-900 text-xs">
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[12%] min-w-[90px] whitespace-nowrap">Section</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[12%] min-w-[95px] whitespace-nowrap">Order Date</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[14%] min-w-[125px] text-center whitespace-nowrap">Next Phase Update</th>
                ...
                {filteredRecords.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="py-24 text-center text-slate-400 font-bold uppercase tracking-wider">
                      No sustained tribunal demands documented
                    </td>
                  </tr>
                ) : (
                  filteredRecords.map((rec, idx) => {
                      ...
                      <tr key={rec.id} className="group hover:bg-indigo-50/10 transition-all border-b border-slate-100 last:border-0">
                        ...
                        <td className={`px-3 py-2 text-center relative whitespace-nowrap ${activeStatusMenuId === rec.id ? "z-50" : "z-0"}`}>
                          <div className="relative inline-block">
                            <button 
                              onClick={() => setActiveStatusMenuId(activeStatusMenuId === rec.id ? null : rec.id)}
                              className="px-2.5 py-1 rounded-lg text-[11px] font-black uppercase tracking-wider bg-rose-50 text-rose-700 border border-rose-200 hover:bg-rose-100 transition-all flex items-center gap-1 cursor-pointer"
                            >
                              <span>Next Phase</span>
                              <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M19 9l-7 7-7-7" /></svg>
                            </button>
                            {activeStatusMenuId === rec.id && (
                              <div className="absolute top-full mt-1 z-50 left-1/2 -translate-x-1/2 w-44 bg-white border border-slate-200 rounded-xl shadow-xl p-1 text-left flex flex-col">
                                <button onClick={() => updateRecordStatus(rec, 'Drop', true)} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-amber-50 text-amber-700 text-left cursor-pointer">Demand Paid</button>
                                <button onClick={() => {
                                  setTargetCategory('HighCourt');
                                  setIsReissueMode(true);
