...
const TribunalDemand: React.FC = () => {
  ...
  return (
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-4 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse table-auto min-w-full">
            <thead className=" sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className=" px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Adverse Ref</th>
                <th className=" px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400">Order Date</th>
                <th className=" px-6 py-5 text-[11px] font-black uppercase tracking-widest text-center">Next Phase Update</th>
                ...
                filteredRecords.map((rec, idx) => {
                  ...
                  return (
                    <tr key={rec.id} className="hover:bg-indigo-50/20 transition-all group text-[12px]">
                      ...
                      <td className=" px-6 py-5 font-black text-red-500 uppercase">{formatDisplayDate(rec.aioDate || rec.oioDate || rec.orderDate || rec.issuedDate)}</td>
      ...
      <NoticeForm isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setIsReissueMode(false); }} onSave={handleSave} onDelete={handleDelete} clients={clients} category={targetCategory} initialData={selectedRecord} isReissue={isReissueMode} />
    </div>
  );
};
