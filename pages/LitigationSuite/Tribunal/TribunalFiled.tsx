...
const TribunalFiled: React.FC = () => {
  ...
  const updateHearingDate = async (record: LitigationRecord, date: string) => {
    try {
      await api.saveTribunalRecord({ ...record, hearingDate: date });
      refreshData();
  ...
  const handleConfirmOutcome = async (e: React.FormEvent) => {
    ...
    try {
      ...
      refreshData();
      toast.success(`Tribunal Appeal marked as ${outcomeStatus === 'Drop' ? 'Relief Granted' : 'Sustained'}`);
    } catch (err) {
  ...
  const handleExportCSV = () => {
    const headers = ['S.No.', 'Trade Name', 'GSTIN', 'Filing/Case No.', 'Appeal U/s', 'Filing Date', 'Hearing Date', 'Status Ref'];
    ...
    exportToCSV(headers, rows, 'GSTAT_Filed.csv');
  };

  const handleExportPDF = () => {
    const headers = ['Trade Name', 'GSTIN', 'Filing/Case No.', 'Appeal U/s', 'Filing Date', 'Hearing Date', 'Status Ref'];
    ...
    printList('GSTAT Filed Appeals', headers, rows);
  };
    ...
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-3 bg-white p-2.5 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 group w-full">
          <input 
            type="text" 
            placeholder="Search filed tribunal matters by Trade Name, GSTIN or Case Ref..." 
            value={search} 
          ...
          <div className="p-4 overflow-y-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredRecords.length === 0 ? (
              <div className="col-span-full py-24 text-center text-slate-400 font-bold uppercase tracking-wider text-xs">
                No filed tribunal matters found
              </div>
            ) : (
                    ...
                    <div className="bg-white p-2.5 rounded-xl border border-slate-200/70 space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center justify-between text-slate-600 gap-2">
                        ...
                        <input 
                          ...
                          className="bg-slate-50 border border-slate-200 rounded px-2 py-0.5 text-xs font-bold text-indigo-600 outline-none focus:ring-1 focus:ring-indigo-500" 
                        />
                      </div>
                    </div>
                      ...
                      <div className="relative">
                        ...
                        {activeStatusMenuId === rec.id && (
                          <div className="absolute bottom-full mb-1 z-50 left-0 w-36 bg-white border border-slate-200 rounded-xl shadow-xl p-1 text-left flex flex-col">
                            <button onClick={() => openOutcomeModal(rec, 'Drop')} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-emerald-50 text-emerald-700 text-left cursor-pointer">Relief Granted</button>
                            <button onClick={() => openOutcomeModal(rec, 'Demand')} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-rose-50 text-rose-700 text-left border-t border-slate-50 cursor-pointer">Sustained</button>
                          </div>
                        )}
                      </div>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1100px]">
              <thead className="sticky top-0 z-30 bg-slate-100">
                <tr className="bg-slate-50 border-b border-slate-200 shadow-sm font-bold uppercase tracking-wider text-slate-900 text-xs">
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[11%] min-w-[90px] whitespace-nowrap">Appeal U/s</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[11%] min-w-[95px] whitespace-nowrap">Filing Date</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[14%] min-w-[125px] whitespace-nowrap">Hearing Date</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[11%] min-w-[95px] whitespace-nowrap">Status Ref</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[12%] min-w-[115px] text-center whitespace-nowrap">Outcome Update</th>
      ...
      {isOutcomeModalOpen && recordToUpdate && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <form onSubmit={handleConfirmOutcome} className="w-full max-w-lg bg-white rounded-[2rem] shadow-2xl flex flex-col overflow-hidden border border-slate-200 animate-in zoom-in-95">
            <div className="p-6 bg-slate-900 text-white flex items-center justify-between shrink-0">
              <div>
                 <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-400 mb-1">GSTAT Outcome Update</p>
                 <h3 className="text-lg font-black truncate">{outcomeStatus === 'Drop' ? 'Relief Granted' : 'Sustained'}</h3>
              </div>
              <button type="button" onClick={() => setIsOutcomeModalOpen(false)} className="h-8 w-8 flex items-center justify-center rounded-xl hover:bg-white/10 transition-colors">
            ...
            <div className="p-6 space-y-4">
               ...
               <div>
                 ...
                 <input type="text" value={outcomeRefNo} onChange={e => setOutcomeRefNo(e.target.value)} placeholder="e.g. GSTAT/ORD/2024/09..." className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl font-bold text-xs outline-none focus:ring-2 focus:ring-indigo-600/10 transition-all font-mono text-slate-900" />
               </div>
            </div>
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex gap-3">
              <button type="button" onClick={() => setIsOutcomeModalOpen(false)} className="flex-1 py-2.5 bg-slate-200 text-slate-700 rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-slate-300 transition-all">Cancel</button>
              <button type="submit" className="flex-1 py-2.5 bg-indigo-600 text-white rounded-xl font-bold text-xs uppercase tracking-wider hover:bg-slate-900 transition-all">Confirm Outcome</button>
            </div>
          </form>
        </div>
      )}
