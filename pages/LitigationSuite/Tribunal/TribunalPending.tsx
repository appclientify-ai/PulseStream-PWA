...
const TribunalPending: React.FC = () => {
  ...
  const updateRecordStatus = async (record: LitigationRecord, newStatus: LitigationStatus) => {
    try {
      ...
      if (newStatus === 'Filed') updated.filedDate = new Date().toISOString().split('T')[0];
  ...
  const getTribunalTiming = (orderDate?: string) => {
    ...
    return { label: `${Math.abs(diff90)} Days Overdue`, color: 'text-red-600', dot: 'bg-red-600' };
  };
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white p-3 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        <div className="flex items-center gap-6 px-4 border-r border-slate-100 hidden md:flex shrink-0">
          <div className="text-center">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">GSTAT Pending</p>
            <p className="text-xl font-black text-slate-900 leading-none">{records.length}</p>
          </div>
        </div>
        <div className="relative flex-1 group w-full">
          <input type="text" placeholder="Search tribunal queue..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1250px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[140px]">Order U/s</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[180px]">Order Ref</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[140px]">Order Date</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[150px]">Deadline</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-center w-[130px]">Status</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                <tr><td colSpan={9} className="py-32 text-center text-slate-300 font-black uppercase tracking-widest text-sm">No Pending Tribunal Records</td></tr>
              ) : (
                filteredRecords.map((rec, idx) => {
                  const timing = getTribunalTiming(rec.orderDate || rec.issuedDate);
                  return (
                    <tr key={rec.id} className="hover:bg-indigo-50/20 transition-all group text-[12px]">
                      ...
                      <td className="px-6 py-5 font-black text-indigo-600 font-mono tracking-widest uppercase">{clients.find(c => c.id === rec.clientId)?.gstProfile?.gstin || 'N/A'}</td>
           ...
           <div className="w-full max-w-2xl bg-white rounded-[2.5rem] shadow-2xl flex flex-col animate-in zoom-in-95 overflow-hidden">
              <div className="px-10 py-8 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
                 <div className="min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">GSTAT Assessment Profile</p>
                 </div>
                 <div className="flex items-center gap-2">
                    <button onClick={() => { setSelectedRecord(viewingRecord); setIsModalOpen(true); }} className="bg-indigo-600 text-white font-black uppercase text-[10px] px-6 py-3 rounded-xl shadow-lg hover:bg-slate-900 transition-all">Edit Matter</button>
              ...
              <div className="p-10 grid grid-cols-2 gap-8 flex-1 overflow-y-auto no-scrollbar">
                 ...
                 <div><p className="text-[10px] font-black uppercase text-slate-400 mb-1">Adverse Order Date</p><p className="text-base font-black text-slate-900">{formatDisplayDate(viewingRecord.orderDate || viewingRecord.issuedDate)}</p></div>
