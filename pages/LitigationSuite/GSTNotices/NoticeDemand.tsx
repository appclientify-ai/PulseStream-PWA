...
const NoticeDemand: React.FC = () => {
  ...
  const { data: pageData, isLoading: isPageLoading } = useModuleData('gst_notice_demand');
  ...
  const [targetCategory, setTargetCategory] = useState<LitigationCategory>('Notice');
  ...
  const refreshData = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['gst_notice_demand'] });
  ...
  const handleExportCSV = () => {
    const headers = ['S.No.', 'Trade Name', 'GSTIN', 'Notice Ref', 'Section', 'Demand Date', 'Status'];
    ...
    exportToCSV(headers, rows, 'GST_Notice_Demands.csv');
  };

  const handleExportPDF = () => {
    const headers = ['Trade Name', 'GSTIN', 'Notice Ref', 'Section', 'Demand Date', 'Status'];
    ...
    printList('GST Notice Sustained Demands', headers, rows);
  };
    ...
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-3 bg-white p-2.5 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 group w-full">
          <input 
            type="text" 
            placeholder="Search sustained demands by Trade Name, GSTIN or Reference..." 
            value={search} 
                  ...
                  <div key={rec.id} className="p-4 bg-slate-50/70 hover:bg-white border border-slate-200 rounded-2xl shadow-xs hover:shadow-md transition-all flex flex-col justify-between space-y-3">
                    ...
                    <div className="bg-white p-2.5 rounded-xl border border-slate-200/70 space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center justify-between text-slate-600">
                        ...
                        <span className="font-medium text-slate-700">{formatDate(rec.issuedDate)}</span>
                      </div>
                      <div className="flex items-center justify-between text-slate-600">
                        ...
                        {activeStatusMenuId === rec.id && (
                          <div className="absolute bottom-full mb-1 z-50 left-0 w-36 bg-white border border-slate-200 rounded-xl shadow-xl p-1 text-left flex flex-col">
                            ...
                            <button onClick={() => {
                              ...
                              setIsModalOpen(true);
                              setActiveStatusMenuId(null);
                            }} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-indigo-50 text-indigo-700 text-left border-t border-slate-50 cursor-pointer">Log Appeal</button>
                            <button onClick={() => updateRecordStatus(rec, 'Filed')} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-slate-50 text-slate-700 text-left border-t border-slate-50 cursor-pointer">Revert Filing</button>
                          </div>
                        )}
                      </div>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1100px]">
              <thead className="sticky top-0 z-30 bg-slate-100">
                <tr className="bg-slate-50 border-b border-slate-200 shadow-sm font-bold uppercase tracking-wider text-slate-900 text-xs">
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[12%] min-w-[110px] whitespace-nowrap">Notice U/s</th>
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[11%] min-w-[110px] whitespace-nowrap">Notice Date</th>
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[12%] min-w-[110px] whitespace-nowrap">Order Date</th>
