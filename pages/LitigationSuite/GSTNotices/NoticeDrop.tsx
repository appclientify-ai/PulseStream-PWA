...
const NoticeDrop: React.FC = () => {
  ...
  const handleExportCSV = () => {
    const headers = ['S.No.', 'Trade Name', 'GSTIN', 'Notice Ref', 'Section', 'Order Date', 'Outcome'];
    const rows = filteredRecords.map((r, idx) => {
      ...
      return [
        ...
        r.isDemandPaid ? 'Paid & Closed' : 'Dropped / Relief'
      ];
    });
    exportToCSV(headers, rows, 'GST_Notice_Dropped.csv');
  };

  const handleExportPDF = () => {
    const headers = ['Trade Name', 'GSTIN', 'Notice Ref', 'Section', 'Order Date', 'Outcome'];
    ...
    printList('GST Dropped Notices', headers, rows);
  };
    ...
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-3 bg-white p-2.5 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 group w-full">
          <input 
            type="text" 
            placeholder="Search dropped notices by Trade Name, GSTIN or Reference..." 
            value={search} 
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-slate-200/60">
                      <div className="relative">
                        <button 
                          ...
                          className={`px-3 py-1 rounded-lg text-xs font-bold uppercase tracking-wider border transition-all flex items-center gap-1 cursor-pointer ${
                            rec.isDemandPaid ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
                          }`}
                        >
                          <span>{rec.isDemandPaid ? 'Paid' : 'Dropped'}</span>
                        ...
                        {activeStatusMenuId === rec.id && (
                          <div className="absolute bottom-full mb-1 z-50 left-0 w-36 bg-white border border-slate-200 rounded-xl shadow-xl p-1 text-left flex flex-col">
                            <button onClick={() => updateRecordStatus(rec, 'Filed')} className="w-full px-2.5 py-1.5 text-xs font-bold uppercase rounded-lg hover:bg-slate-50 text-slate-700 text-left cursor-pointer">Revert to Filed</button>
                          </div>
                        )}
                      </div>
            ...
            <table className="w-full text-left border-collapse table-fixed min-w-[1100px]">
              <thead className="sticky top-0 z-30 bg-slate-100">
                <tr className="bg-slate-50 border-b border-slate-200 shadow-sm font-bold uppercase tracking-wider text-slate-900 text-xs">
                  ...
                  <th className="sticky top-0 z-30 bg-slate-100 px-3 py-2 border-b border-slate-200 w-[12%] min-w-[110px] whitespace-nowrap">Section</th>
                      ...
                      <tr key={rec.id} className="group hover:bg-indigo-50/10 transition-all border-b border-slate-100 last:border-0">
                        ...
                        <td className={`px-3 py-2 text-center relative whitespace-nowrap ${activeStatusMenuId === rec.id ? "z-50" : "z-0"}`}>
                          <div className="relative inline-block">
                            <button 
                              onClick={() => setActiveStatusMenuId(activeStatusMenuId === rec.id ? null : rec.id)}
                              className={`px-2.5 py-1 rounded-lg text-[11px] font-black uppercase tracking-wider border transition-all flex items-center gap-1 cursor-pointer ${
                                rec.isDemandPaid ? 'bg-amber-50 text-amber-700 border-amber-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'
                              }`}
                            >
                              <span>{rec.isDemandPaid ? 'Paid' : 'Dropped'}</span>
