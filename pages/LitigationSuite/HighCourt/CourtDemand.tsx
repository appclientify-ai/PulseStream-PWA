...
const CourtDemand: React.FC = () => {
  ...
  const updateRecordStatus = async (record: LitigationRecord, newStatus: LitigationRecord['status'], isPaid: boolean = false) => {
    try {
      ...
      fetchAll();
    } catch (err) { alert("Outcome update failed."); }
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1250px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[180px]">Judgment Ref</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[140px]">Judgment Date</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-center w-[160px]">Outcome</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-right w-[100px]">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                <tr><td colSpan={8} className="py-32 text-center text-slate-300 font-black uppercase tracking-widest text-sm">No Unfavorable Court Orders Recorded</td></tr>
              ) : (
