...
const CourtDrop: React.FC = () => {
  ...
  const handleExportCSV = () => {
    const headers = ['S.No.', 'Trade Name', 'GSTIN', 'Filing/WP No.', 'Matter U/s', 'Order Date', 'Outcome'];
    const rows = filteredRecords.map((r, idx) => {
      ...
      return [
        ...
        r.isDemandPaid ? 'Paid / Settled' : 'Relief Granted'
      ];
    });
    exportToCSV(headers, rows, 'HighCourt_Reliefs.csv');
  };

  const handleExportPDF = () => {
    const headers = ['Trade Name', 'GSTIN', 'Filing/WP No.', 'Matter U/s', 'Order Date', 'Outcome'];
    ...
    printList('High Court Relief Orders', headers, rows);
  };
    ...
    <div className="animate-in fade-in duration-500 h-full flex flex-col space-y-3 overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-3 bg-white p-2.5 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 group w-full">
          <input 
            type="text" 
            placeholder="Search High Court reliefs by Trade Name, GSTIN or Case Ref..." 
            value={search} 
      ...
      <div className="flex-1 bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col">
        {filteredRecords.length === 0 ? (
          <div className="py-32 text-center text-slate-300 font-black uppercase tracking-widest text-sm">
            No High Court Reliefs Archived
          </div>
        ) : viewMode === 'table' ? (
          /* Table View */
                ...
                {filteredRecords.map((rec, idx) => {
                  ...
                  return (
                    <tr key={rec.id} className="hover:bg-emerald-50/20 transition-all group text-[12px]">
                      ...
                      <td className="px-5 py-3.5 font-black text-emerald-600 uppercase">
                      ...
                      <td className="px-5 py-3.5 text-center">
                        <span className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider border ${
                          ...
                          {rec.isDemandPaid ? 'Paid / Settled' : 'Relief Granted'}
                        </span>
                      </td>
          ...
          <div className="p-4 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3 overflow-y-auto no-scrollbar flex-1">
            {filteredRecords.map((rec, idx) => {
              const client = clients.find(c => c.id === rec.clientId);
              return (
                <div key={rec.id} className="bg-slate-50/70 border border-slate-200/80 rounded-2xl p-4 flex flex-col justify-between hover:shadow-md hover:border-emerald-200 transition-all group">
                  <div className="space-y-3">
                    ...
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase ${
                        rec.isDemandPaid ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                      }`}>
                        {rec.isDemandPaid ? 'Paid' : 'Relief'}
                      </span>
                    </div>
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center justify-between text-slate-600">
                        ...
                        <span className="font-bold text-emerald-600">{formatDate(rec.orderDate || rec.tioDate || rec.issuedDate)}</span>
                      </div>
                    </div>
                  ...
                  {/* Card Footer Actions */}
                  <div className="pt-3 mt-3 border-t border-slate-200/60 flex items-center justify-end gap-1.5">
                    {client && (
                      <>
                        <GSTViewIcon client={client} onDataChange={refreshData} />
                        <button
