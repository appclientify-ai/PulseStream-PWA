...
const MSMERegistration: React.FC = () => {
  ...
  const fetchRecords = async () => {
    setIsLoading(true);
    try {
      const data = await mockBackend.getMSMERegistrations();
      setRecords(data);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleInlineUpdate = async (id: string, field: keyof MSMERegistrationRecord, value: any) => {
    ...
    await mockBackend.saveMSMERegistration(newRecord);
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: records.filter(r => !['Completed', 'Failed'].includes(r.status)).length
    };
  }, [records]);

  const handleDelete = async (id: string) => {
    if (confirm('Permanently delete this MSME record?')) {
      await mockBackend.deleteMSMERegistration(id);
      fetchRecords();
    }
  };
  ...
  return (
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-500 max-w-full mx-auto w-full overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white p-3 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        <div className="flex items-center gap-6 px-4 border-r border-slate-100 hidden md:flex shrink-0">
          <div className="text-center">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Registrations</p>
        ...
        <div className="relative flex-1 w-full group">
          <input type="text" placeholder="Search Client, Mobile or Udyam No..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1200px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[220px]">Client</th>
                ...
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-center w-[180px] relative">
                  ...
                  {isStatusFilterOpen && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-48 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-1 animate-in zoom-in-95 text-left">
                      ...
                      {['Pending', 'Data Requested', 'In Progress', 'Completed', 'Failed'].map(st => (
                ...
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[200px]">Udyam Number</th>
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[250px]">Remarks</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                <tr><td colSpan={8} className="py-32 text-center text-slate-300 font-black uppercase tracking-[0.2em] text-sm">No MSME records found</td></tr>
              ) : (
                  ...
                  <tr key={rec.id} className="hover:bg-slate-50/50 transition-all group text-[12px]">
                    ...
                    <td className="px-4 py-5">
                       <input 
                         ...
                         onChange={e => handleInlineUpdate(rec.id, 'udyamNumber', e.target.value.toUpperCase())}
                         placeholder="Add Udyam No..."
      ...
      <MSMEForm 
        ...
        onSave={async (data) => {
          await mockBackend.saveMSMERegistration(data);
          setIsFormOpen(false);
          fetchRecords();
        }} 
