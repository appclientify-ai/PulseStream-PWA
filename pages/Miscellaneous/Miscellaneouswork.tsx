...
const Miscellaneouswork: React.FC = () => {
  ...
  const fetchRecords = async () => {
    setIsLoading(true);
    try {
      const data = await mockBackend.getMiscWork();
      setRecords(data);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleInlineUpdate = async (id: string, field: keyof MiscWorkRecord, value: any) => {
    ...
    await mockBackend.saveMiscWork(newRecord);
  ...
  const filteredRecords = useMemo(() => {
    ...
    return list.filter(r => 
      ...
      (r.assignedTo && r.assignedTo.toLowerCase().includes(s))
    );
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: records.filter(r => ['Pending', 'In Progress'].includes(r.status)).length,
  ...
  const handleDelete = async (id: string) => {
    if (confirm('Permanently delete this work item?')) {
      await mockBackend.deleteMiscWork(id);
      fetchRecords();
    }
  };
    ...
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-500 max-w-full mx-auto w-full overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white p-3 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        ...
        <div className="relative flex-1 w-full group">
          <input type="text" placeholder="Search Client, Description, Staff..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1500px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[240px]">Work Description</th>
                
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-center w-[160px] relative">
                  ...
                  {isStatusFilterOpen && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-44 bg-white border border-slate-200 rounded-xl shadow-xl z-50 p-1 animate-in zoom-in-95 text-left">
                ...
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[160px]">Assigned To</th>
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[140px]">Start Date</th>
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[140px]">Done Date</th>
                <th className="px-4 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[200px]">Remarks</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                <tr><td colSpan={10} className="py-32 text-center text-slate-300 font-black uppercase tracking-[0.2em] text-sm">No work items documented</td></tr>
              ) : (
                filteredRecords.map((rec, idx) => (
                  <tr key={rec.id} className="hover:bg-slate-50/50 transition-all group text-[12px]">
                    ...
                    <td className="px-4 py-5">
                      <p className="font-bold text-slate-500 uppercase truncate" title={rec.description}>{rec.description}</p>
                    </td>
                    ...
                    <td className="px-4 py-5">
                       <input 
                         ...
                         onChange={e => handleInlineUpdate(rec.id, 'assignedTo', e.target.value.toUpperCase())}
                         placeholder="Staff..."
                         className="w-full bg-transparent border-none focus:bg-white focus:ring-2 focus:ring-indigo-100 rounded-lg px-2 py-1.5 font-black text-indigo-600 uppercase transition-all"
                       />
                    </td>
                    <td className="px-4 py-5">
                       <input 
                         type="date" 
                         value={rec.startDate || ''} 
                         onChange={e => handleInlineUpdate(rec.id, 'startDate', e.target.value)}
                         className="w-full bg-transparent border-none focus:bg-white focus:ring-2 focus:ring-indigo-100 rounded-lg px-2 py-1.5 font-black text-slate-700 uppercase transition-all cursor-pointer"
                       />
                    </td>
                    <td className="px-4 py-5">
                       <input 
      ...
      <WorkForm 
        ...
        onSave={async (data) => {
          await mockBackend.saveMiscWork(data);
          setIsFormOpen(false);
          fetchRecords();
        }} 
