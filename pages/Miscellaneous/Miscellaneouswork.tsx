          ...
          <table className="w-full text-left border-collapse table-auto overflow-hidden min-w-[1200px]">
            <thead className="whitespace-nowrap sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="whitespace-nowrap px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[300px]">Task Description</th>
                ...
                <th className="whitespace-nowrap px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[140px]">Staff Head</th>
                <th className="whitespace-nowrap px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[130px]">Logged Date</th>
                <th className="whitespace-nowrap px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-right w-[110px]">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
              {filteredRecords.length === 0 ? (
                <tr><td colSpan={7} className="whitespace-nowrap py-32 text-center text-slate-300 font-black uppercase tracking-widest text-sm">No miscellaneous tasks archived</td></tr>
              ) : (
                filteredRecords.map((rec, idx) => (
                  <tr key={rec.id} className="hover:bg-indigo-50/20 transition-all group text-[12px]">
                    ...
                    <td className="whitespace-nowrap px-6 py-5 font-bold text-slate-600 uppercase truncate" title={rec.description}>{rec.description}</td>
                    ...
                    <td className="whitespace-nowrap px-6 py-5 font-black text-slate-500 uppercase truncate">{rec.assignedTo || 'Unassigned'}</td>
                    <td className="whitespace-nowrap px-6 py-5 font-black text-slate-400 uppercase">{formatDate(rec.startDate)}</td>
