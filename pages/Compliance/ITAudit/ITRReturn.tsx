          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1550px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-200 shadow-sm text-[14px] font-bold uppercase tracking-widest text-slate-900">
                <th className="px-4 py-3 w-[90px]">ID no.</th>
                       ...
                       <div className="flex items-center gap-2 group/pass">
                          {isEditingPass ? (
                            <div className="flex items-center gap-1">
                               <input autoFocus value={newPasswordValue} onChange={e => setNewPasswordValue(e.target.value)} onBlur={() => saveQuickPassword(client)} onKeyDown={e => e.key === 'Enter' && saveQuickPassword(client)} className="bg-white border border-indigo-200 rounded px-2 h-7 text-[11px] font-black w-24 outline-none" />
                            </div>
                          ) : (
                            <>
        ...
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-slate-900/70 backdrop-blur-xl p-4 animate-in fade-in duration-200">
          <div className="w-full max-w-5xl max-h-[95vh] bg-white rounded-[3rem] shadow-2xl flex flex-col overflow-hidden border border-slate-200">
            <div className="px-10 py-8 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
              <div className="min-w-0"><h2 className="text-2xl font-black text-slate-900 uppercase truncate">{selectedClient.legalName}</h2><p className="text-sm font-bold text-slate-500 uppercase tracking-widest mt-1">IT Dossier • AY {selectedAY}</p></div>
            ...
            <div className="p-10 overflow-y-auto no-scrollbar flex-1 space-y-12">
               <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-1"><p className="text-[9px] font-black uppercase text-slate-400">PAN Identification</p><p className="text-base font-black text-indigo-600 font-mono tracking-widest uppercase">{selectedClient.itProfile?.pan}</p></div>
                  <div className="space-y-1"><p className="text-[9px] font-black uppercase text-slate-400">Compliance Status</p><p className="text-base font-black text-emerald-600 uppercase">{selectedClient.status}</p></div>
               </div>
               <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100 grid grid-cols-2 md:grid-cols-4 gap-8">
                  <div><p className="text-[9px] font-black uppercase text-slate-400 mb-1">Father's Name</p><p className="text-sm font-black text-slate-900 uppercase">{selectedClient.itProfile?.fatherName || '---'}</p></div>
                  <div><p className="text-[9px] font-black uppercase text-slate-400 mb-1">DOB / Incorporation</p><p className="text-sm font-black text-slate-900 uppercase">{selectedClient.itProfile?.dob || '---'}</p></div>
               ...
               <div className="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                  <p className="text-[9px] font-black uppercase text-slate-400 mb-2">Vault Internal Remarks</p>
                  <p className="text-xs font-medium text-slate-600 italic leading-relaxed">{selectedClient.remarks || 'No legacy notes documented.'}</p>
               </div>
            </div>
