              ...
              <>
                ...
                {iffDueDate && (
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-indigo-50 border border-indigo-200/90 text-[10px] sm:text-[11px] font-black text-indigo-700 whitespace-nowrap shrink-0 flex-shrink-0 shadow-2xs">
                    <span className="text-indigo-600 font-bold uppercase">IFF/R1 Due:</span>
                    <span className="font-mono font-bold text-indigo-950">{formatISOToDDMMYYYY(iffDueDate)}</span>
                  </div>
                )}
                ...
                {isQuarterEnd && q3bDueDate && (
                  <div className="flex items-center gap-1 px-2 py-0.5 rounded-lg bg-emerald-50 border border-emerald-200/90 text-[10px] sm:text-[11px] font-black text-emerald-700 whitespace-nowrap shrink-0 flex-shrink-0 shadow-2xs">
                    ...
                    <span className="font-mono font-bold text-emerald-950">{formatISOToDDMMYYYY(q3bDueDate)}</span>
                  </div>
                )}
              </>
            )}
          ...
          <table className={`w-full text-left border-collapse min-w-[1000px] quarterly-returns-table ${isAllQuartersMode ? 'min-w-[1450px]' : ''}`}>
                        ...
                        >
                          {['All', '3B Filed', '3B Challan', '3B Pending', 'R1 Filed', 'R1 Pending'].map(f => (
                            <button 
                              ...
                              className="w-full text-left px-2.5 py-1.5 text-[var(--app-font-size)] font-black uppercase rounded-lg hover:bg-slate-50 text-slate-700"
                            >
            ...
            <tbody className="divide-y divide-slate-100">
              {groupedClients.map(({ sector, clients: sectorClients }) => (
                ...
                <tr>
                  <td colSpan={isAllQuartersMode ? 10 : 10} className="sticky top-[27px] z-20 bg-slate-200/95 backdrop-blur-md font-bold text-slate-800 py-0.5 px-3 uppercase text-[10px] tracking-widest border-y border-slate-300 shadow-xs">{sector} ({sectorClients.length})</td>
                </tr>
                              ...
                              {qConfig.months.map(m => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`px-0.5 py-0.5 rounded text-[9px] font-black uppercase border text-center transition-all ${
                                      mSt.r1
                            ...
                            {(() => {
                              ...
                              return (
                                <button
                                  ...
                                  className={`w-full px-1 py-0.5 rounded text-[9px] font-black uppercase border flex items-center justify-between gap-0.5 transition-all ${
                                    label3B === 'Filed'
                                ...
                                >
                                  <span className="text-[9px] text-slate-400">3B</span>
                        ...
                        <td className="px-3 py-1.5 text-center w-[11%] min-w-[110px] whitespace-nowrap">
                           {activeQuarterConfig ? (
                             (() => {
                               ...
                               const label3B = getStatusLabel(st3B.r3b);
                               return (
                                 <button onClick={() => toggleStatus(client.id, 'r3b', r3bPeriod)} className={`px-2.5 py-0.5 rounded-full text-[var(--app-font-size)] font-black uppercase border flex items-center justify-center gap-1 mx-auto transition-all ${
                                    label3B === 'Filed' 
                                      ? 'bg-emerald-100 text-emerald-700 border-emerald-200 hover:bg-emerald-200' 
                                      : label3B === 'Challan' 
                                        ? 'bg-amber-100 text-amber-800 border-amber-300 hover:bg-amber-200' 
