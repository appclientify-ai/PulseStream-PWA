...
import { Client, LitigationStatus } from '../../../types';
                ...
                return (
                  <tr key={client.id} className="hover:bg-indigo-50/10 transition-all group border-b border-slate-50 last:border-0 h-[44px]">
                    ...
                    <td className="px-[5.5px] py-[2px] text-center">
                       <button onClick={() => toggleStatus(client.id, 'r1')} className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border flex items-center justify-center gap-1 mx-auto ${st.r1 ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                          {st.r1 ? 'Filed' : 'Pending'}
                    ...
                    <td className="px-[5.5px] py-[2px] text-center">
                       <button onClick={() => toggleStatus(client.id, 'r3b')} className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border flex items-center justify-center gap-1 mx-auto ${st.r3b ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
      ...
      {activeActionsId && selectedClient && (
        <div ref={actionsRef} style={{ top: menuPosition.top, left: menuPosition.left }} className="fixed w-64 bg-white border border-slate-200 rounded-[1.5rem] shadow-2xl z-[9999] p-2 animate-in zoom-in-95 origin-top-right text-left">
           <div className="px-3 py-2 border-b border-slate-50 mb-1">
              <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Client Ops</p>
           ...
           <button onClick={() => { setIsLoginBoxOpen(true); setActiveActionsId(null); }} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-indigo-50 rounded-xl transition-colors text-left group">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-700">Portal bridge</span>
           </button>
           <button onClick={() => { setActiveActionsId(null); }} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 rounded-xl transition-colors text-left group">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">View profile</span>
           </button>
           <button onClick={() => { if(confirm('Delete from vault?')) api.deleteClient(selectedClient.id).then(fetchClients); setActiveActionsId(null); }} className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-red-50 rounded-xl transition-colors text-left group">
