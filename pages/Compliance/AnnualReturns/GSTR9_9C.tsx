...
const GSTR9_9C: React.FC = () => {
  ...
  const { getStatus, toggleStatus, getDueDate, updateDueDate, is9CApplicable } = useGSTR9Logic(selectedYear);
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setClients(data.filter(c => c.gstProfile?.regType === 'Regular'));
  ...
  const stats = useMemo(() => {
    ...
    const gstr9Filed = filteredClients.filter(c => getStatus(c.id).gstr9).length;
    const gstr9cFiled = filteredClients.filter(c => getStatus(c.id).gstr9c).length;
    return { total, gstr9Filed, gstr9cFiled };
    ...
    <div className="flex flex-col h-full space-y-8 animate-in fade-in duration-500 overflow-hidden">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <ModuleStatCard 
              ...
              icon="📑" 
              stats={[
                  { label: 'Regular Entities', value: stats.total },
          ...
          <ModuleStatCard 
              ...
              icon="📤" 
              stats={[
                  { label: 'Filed', value: stats.gstr9Filed, color: 'text-indigo-600' },
                  { label: 'Ratio', value: `${Math.round((stats.gstr9Filed/stats.total)*100 || 0)}%` }
              ]}
              chartData={{ value: stats.gstr9Filed, total: stats.total }}
          />
          <ModuleStatCard 
              title="GSTR-9C Reconciliation" 
              icon="🛡️" 
              stats={[
                  { label: 'Certified', value: stats.gstr9cFiled, color: 'text-emerald-600' },
                  { label: 'Eligible', value: clients.filter(c => is9CApplicable(c.id)).length }
              ]}
              chartData={{ value: stats.gstr9cFiled, total: clients.filter(c => is9CApplicable(c.id)).length }}
          />
      </section>
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white p-3 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        <div className="relative flex-1 group w-full">
          <input type="text" placeholder="Search regular taxpayers in GSTR-9 vault..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1300px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-center w-[120px]">GSTR-9</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 text-center w-[120px]">GSTR-9C</th>
                <th className="px-6 py-5 text-[11px] font-black uppercase tracking-widest text-slate-400 w-[180px]">Status Update</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredClients.map((c, idx) => {
                ...
                const is9CReq = is9CApplicable(c.id);
                return (
                  <tr key={c.id} className="group hover:bg-indigo-50/20 transition-all text-[12px]">
                    ...
                    <td className="px-6 py-5 text-center">
                       <button onClick={() => toggleStatus(c.id, 'gstr9')} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border ${st.gstr9 ? 'bg-indigo-100 text-indigo-700 border-indigo-200' : 'bg-slate-100 text-slate-400 border-slate-200 hover:bg-slate-200'}`}>
                         {st.gstr9 ? 'Filed' : 'Pending'}
                       </button>
                    </td>
                    <td className="px-6 py-5 text-center">
                       <button disabled={!is9CReq} onClick={() => toggleStatus(c.id, 'gstr9c')} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border ${!is9CReq ? 'opacity-20 cursor-not-allowed' : st.gstr9c ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-slate-100 text-slate-400 border-slate-200 hover:bg-slate-200'}`}>
                         {!is9CReq ? 'N/A' : st.gstr9c ? 'Filed' : 'Pending'}
                       </button>
                    </td>
                    <td className="px-6 py-5">
                       <p className="text-[10px] font-black text-slate-500 uppercase">{st.gstr9Date ? `9: ${st.gstr9Date}` : ''}</p>
                       <p className="text-[10px] font-black text-slate-500 uppercase">{st.gstr9cDate ? `9C: ${st.gstr9cDate}` : ''}</p>
                    </td>
