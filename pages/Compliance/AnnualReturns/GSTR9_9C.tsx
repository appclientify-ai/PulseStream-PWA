...
import { useGSTR9Logic } from './GSTR9_9Clogic.tsx';
...
const GSTR9_9C: React.FC = () => {
  ...
  const stats = useMemo(() => {
    const gstr9Filed = trackedClients.filter(c => getStatus(c.id).gstr9).length;
    return { total: trackedClients.length, gstr9Filed };
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1300px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-4 py-5 text-[12px] font-black uppercase tracking-widest text-slate-400 text-center w-[130px]">GSTR-9</th>
                <th className="px-4 py-5 text-[12px] font-black uppercase tracking-widest text-slate-400 text-center w-[130px]">GSTR-9C</th>
                <th className="px-4 py-5 text-[12px] font-black uppercase tracking-widest text-slate-400 w-[160px]">Login ID</th>
              ...
              {filteredDisplayList.map((client, idx) => {
                ...
                return (
                  <tr key={client.id} className="hover:bg-slate-50/50 transition-all">
                    ...
                    <td className="px-4 py-5"><p className="text-[12px] font-black text-slate-900 uppercase truncate">{client.tradeName || client.legalName}</p></td>
                    <td className="px-4 py-5 font-black text-indigo-600 font-mono text-[11px]">{client.gstProfile?.gstin}</td>
                    <td className="px-4 py-5 text-center">
                      <button onClick={() => toggleStatus(client.id, 'gstr9')} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all ${st.gstr9 ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-400'}`}>
                        {st.gstr9 ? 'Filed' : 'Pending'}
                      </button>
                    </td>
                    <td className="px-4 py-5 text-center">
                       <button disabled={!is9CReq} onClick={() => toggleStatus(client.id, 'gstr9c')} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all ${!is9CReq ? 'opacity-20' : st.gstr9c ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-400'}`}>
                        {!is9CReq ? 'Exempt' : st.gstr9c ? 'Filed' : 'Pending'}
                      </button>
                    </td>
                    <td className="px-4 py-5"><span className="text-[10px] font-black text-slate-700 truncate">{client.gstProfile?.username}</span></td>
                    <td className="px-4 py-5 text-right">
                       <button onClick={() => { setSelectedClient(client); setIsDetailModalOpen(true); }} className="h-8 w-8 rounded-lg bg-slate-50 border border-slate-100 text-slate-400 hover:text-indigo-600 flex items-center justify-center shadow-sm ml-auto">
