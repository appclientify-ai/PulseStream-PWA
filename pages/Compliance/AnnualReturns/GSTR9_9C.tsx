...
const GSTR9_9C: React.FC = () => {
  ...
  const { getStatus, toggleStatus, updateDueDate, getDueDate, is9CApplicable } = useGSTR9Logic(selectedYear);
    ...
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-500 overflow-hidden">
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white p-3 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        <div className="flex items-center gap-3 px-4 border-r border-slate-100 hidden md:flex shrink-0">
            <span className="text-[10px] font-black uppercase text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full">{filteredClients.length} Audit Files</span>
        </div>
        <div className="relative flex-1 group w-full">
          <input type="text" placeholder="Search audit portfolio..." value={search} onChange={e => setSearch(e.target.value)}
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[1300px]">
            <thead className="sticky top-0 z-20">
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-center w-[120px]">GSTR-9</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-center w-[120px]">GSTR-9C</th>
                <th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 w-[180px]">Vault ID</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredClients.map((client, idx) => {
                ...
                const req9C = is9CApplicable(client.id);
                return (
                  <tr key={client.id} className="hover:bg-indigo-50/20 transition-all text-[12px]">
                    ...
                    <td className="px-6 py-4 text-center">
                       <button onClick={() => toggleStatus(client.id, 'gstr9')} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border ${st.gstr9 ? 'bg-indigo-100 text-indigo-700 border-indigo-200 shadow-sm' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                         {st.gstr9 ? 'Filed' : 'Pending'}</button>
                    </td>
                    <td className="px-6 py-4 text-center">
                       <button disabled={!req9C} onClick={() => toggleStatus(client.id, 'gstr9c')} className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest transition-all border ${!req9C ? 'opacity-20 cursor-not-allowed' : st.gstr9c ? 'bg-emerald-100 text-emerald-700 border-emerald-200 shadow-sm' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                         {!req9C ? 'Exempt' : st.gstr9c ? 'Filed' : 'Pending'}</button>
                    </td>
