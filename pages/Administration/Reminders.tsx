...
interface UnifiedDeadline {
  ...
  status: string;
  origin: 'litigation' | 'work' | 'food_license';
  expiryDate?: string;
}
...
const Reminders: React.FC = () => {
  ...
  const [filter, setFilter] = useState<'All' | 'Litigation' | 'Misc Work' | 'Food License'>('All');
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      overdue: totalDeadlines.filter(d => getDaysLeft(d.date) < 0).length,
      dueIn15Days: totalDeadlines.filter(d => {
        const dl = getDaysLeft(d.date);
        const threshold = d.origin === 'food_license' ? 60 : 15;
        return dl >= 0 && dl <= threshold;
      }).length,
    ...
    <div className="flex flex-col h-full space-y-4 animate-in fade-in duration-500 max-w-full mx-auto w-full overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white p-3 rounded-[1.5rem] border border-slate-200 shadow-sm shrink-0">
        <div className="flex items-center justify-between sm:justify-start gap-4 md:gap-6 px-3 py-1.5 border-b md:border-b-0 md:border-r border-slate-100 shrink-0 w-full md:w-auto overflow-x-auto no-scrollbar">
          ...
          <div className="text-center border-l border-slate-100 pl-4 md:pl-6">
            <p className="text-[9px] font-black text-amber-500 uppercase tracking-widest mb-1">Due ≤15 Days</p>
            <p className="text-lg md:text-xl font-black text-amber-600 leading-none">{stats.dueIn15Days}</p>
          </div>
        </div>
        ...
        <div className="flex items-center bg-slate-100 p-1 rounded-xl shrink-0 overflow-x-auto no-scrollbar">
          {['All', 'Litigation', 'Misc Work', 'Food License'].map(f => (
            <button key={f} onClick={() => setFilter(f as any)}
              className={`px-3 py-2 text-[10px] font-black uppercase tracking-widest rounded-lg transition-all whitespace-nowrap ${filter === f ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}>
              {f}
            </button>
          ))}
        </div>
                  ...
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-all group">
                    ...
                    <td className=" px-4 py-5 font-black text-slate-700 uppercase">
                      <div>
                        ...
                        {item.expiryDate && (
                          <p className="text-[9px] font-bold text-slate-400 normal-case">Exp: {formatDate(item.expiryDate)}</p>
                        )}
                      </div>
                    </td>
                    <td className=" px-4 py-5">
                      {dl < 0 ? (
                        <span className="px-2 py-1 rounded-lg text-[10px] font-black uppercase bg-red-100 text-red-700 border border-red-200 shadow-2xs inline-flex items-center gap-1">
                          <span className="h-1.5 w-1.5 rounded-full bg-red-600 animate-ping" />
                          {item.origin === 'food_license' ? `Expired (${Math.abs(dl)}d) • Late Fees Apply` : `${Math.abs(dl)} days overdue`}
                        </span>
                      ) : dl <= (item.origin === 'food_license' ? 60 : 15) ? (
                        <span className="px-2 py-1 rounded-lg text-[10px] font-black uppercase bg-amber-100 text-amber-800 border border-amber-300 shadow-2xs animate-pulse inline-flex items-center gap-1">
                          <span className="h-1.5 w-1.5 rounded-full bg-amber-600 animate-ping" />
                          {dl} days left {item.origin === 'food_license' ? '(Renewal Window)' : ''}
                        </span>
                      ) : (
