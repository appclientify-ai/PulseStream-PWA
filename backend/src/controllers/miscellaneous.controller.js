...
export const getRecords = (collectionName, oldCategoryName) => async (req, res) => {
  try {
...
export const createRecord = (collectionName) => async (req, res) => {
  try {
    const collection = getCollection(collectionName);
    const newRecord = {
      ...req.body,
...
export const updateRecord = (collectionName) => async (req, res) => {
  const { id } = req.params;
  try {
    const collection = getCollection(collectionName);
...
export const deleteRecord = (collectionName) => async (req, res) => {
  const { id } = req.params;
  try {
    const collection = getCollection(collectionName);
    const query = buildQuery(id, req.user._id);
