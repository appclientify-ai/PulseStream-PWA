...
files.forEach(file => {
  ...
  const targetRegex = /<button onClick=\{\(\) => \{\s*const text = [^}]+?\}\}\s*className="w-full flex items-center gap-3 px-3 py-2.5 hover:bg-slate-50 rounded-xl transition-colors text-left group">.*?<\/button>/s;
  ...
  const replacement = `
           <button onClick={() => { 
             ...
             navigator.clipboard.writeText(text).then(() => {
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Copy Credentials</span>
           </button>
           <button onClick={() => { 
             const text = \`Trade Name: \${selectedClient.tradeName || ''}\\nLegal Name: \${selectedClient.legalName || ''}\\nGSTIN: \${selectedClient.gstProfile?.gstin || ''}\\nUser ID: \${selectedClient.gstProfile?.username || ''}\\nPassword: \${selectedClient.gstProfile?.password || ''}\`;
